...
def invalidate_analytics_on_feeding_save(sender, instance, **kwargs):
    ...
    Uses transaction.on_commit() to defer invalidation until after the
    database transaction commits, preventing race conditions where the
    timeline cache is cleared but the database hasn't been updated yet.
    """
    transaction.on_commit(lambda: invalidate_child_analytics(instance.child_id))
